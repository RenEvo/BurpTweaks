package com.renevo.burptweaks.lib;

public final class Constants {

	public static final String MOD_ID = "burptweaks";
	
	public static final String MOD_VERSION = "1.1";
	
	public static final String MOD_NAME = "BurpTweaks";
	
	public static final String MOD_CHANNEL_NAME = MOD_ID;
	
	// be sure to make these lower case, the configuration API's are not consistent when reading/writing the names
	public static final String CONFIG_CATEGORY_GENERAL = "general";
	
	public static final String CONFIG_CATEGORY_TWEAKS = "tweaks";
	
	public static final String CONFIG_CATEGORY_RECIPES = "recipes";
	
	public static final String CONFIG_CATEGORY_INTEGRATION = "integration";
	
	public static final String CONFIG_CATEGORY_SPAWNRULES = "spawn restrictions";
	
	
}
